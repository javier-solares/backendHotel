...
namespace dw_backend.Controllers.Administracion.Cliente
{
    ...
    public class ClienteController : ControllerBase
    {
        ...
        public ClienteController(IConfiguration configuration)
        {
            ...
            //_commandText = _configuration.GetValue<string>("DataBase:StoredProcedures:Cliente");
        }
        ...
        public IActionResult All()
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("adm.crudCliente", conn))
                    {
        ...
        public IActionResult Create(dynamic request)
        {
            try
            {
                using (SqlConnection conn = MainConnection.Connection(_configuration))
                {
                    using (SqlCommand cmd = new SqlCommand("adm.crudCliente", conn))
                    {
                        cmd.Parameters.AddWithValue("@persona", (Int32)request.persona);
                        cmd.Parameters.AddWithValue("@email", (string)request.email);
                        cmd.Parameters.AddWithValue("@telefono", (string)request.telefono);
                        cmd.Parameters.AddWithValue("@fechaRegistro", (string)request.fechaRegistro);
