...
namespace dw_backend.Controllers.Administracion.Reserva
{
    ...
    public class ReservaController : ControllerBase
    {
        ...
        public IActionResult All()
        {
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand("adm.crudReserva", conn))
                    {
        ...
        public IActionResult Create(dynamic request)
        {
            try
            {
                using (SqlConnection conn = MainConnection.Connection(_configuration))
                {
                    using (SqlCommand cmd = new SqlCommand("adm.crudReserva", conn))
                    {
                        cmd.Parameters.AddWithValue("@cliente", (Int32)request.cliente);
                        cmd.Parameters.AddWithValue("@habitaciones", (Int32)request.habitaciones);
                        cmd.Parameters.AddWithValue("@fechaIngreso", (string)request.fechaIngreso);
                        cmd.Parameters.AddWithValue("@fechaSalida", (string)request.fechaSalida);
                        cmd.Parameters.AddWithValue("@comentario", (string)request.comentario);
